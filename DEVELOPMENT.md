...
Pause mutes the recording's own capture endpoint through an inline C# shim run by `powershell.exe` (`IMMDeviceEnumerator` → `IAudioEndpointVolume`). The session microphone's name goes in the `DW_MIC_NAME` environment variable (never the command line); the shim matches it against the active endpoints' friendly names (exactly, or the only one it is a prefix of) and falls back to the default endpoint. whisper-stream reports the same friendly names. The COM declarations need `[ComImport]`, and `IAudioEndpointVolume` has **eleven** methods before `SetMute`; getting either wrong fails the cast or calls the wrong vtable slot. During a session the state is polled every second; a change from outside the app pauses or resumes. Each call starts PowerShell and compiles the shim (~0.8 s), so polling is limited to active sessions.
